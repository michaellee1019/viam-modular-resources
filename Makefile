...
install-tools:
	ssh $(target) "sudo apt-get update; sudo apt-get install -y python3-pip python3-venv i2c-tools; pip install ; pip install -U viam-sdk; sudo raspi-config nonint do_i2c 0; sudo reboot"
...
robot-copy:
	...
	ssh $(target) "rm -rf ~/src; rm -rf ~/test; rm -f meta.json;"
	scp -r src $(target):~/
	scp -r test $(target):~/
